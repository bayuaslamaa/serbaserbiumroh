...
import { VideoDocGrid } from '@/packages/badalin/presentation/view/video-doc-grid';
import {
  badalSteps,
  badalVideos,
  badalinHero,
  badalinPrice,
  isPlaceholderVideo,
} from '@/packages/badalin/domain/content';
import { badalConsultHref } from '@/packages/layanan/domain/catalog';

const readyVideoCount = badalVideos.filter((video) => !isPlaceholderVideo(video.youtubeId)).length;

const allVideosReady = readyVideoCount === badalVideos.length;

export const BadalinView = () => {
  return (
    <div className="mx-auto max-w-6xl pb-20">
      ...
      >
        ...
        <p className="mx-auto mb-6 mt-2.5 max-w-[480px] text-sm leading-relaxed text-text-muted">
          Ceritakan untuk siapa badal ini — tim kami balas dengan jadwal terdekat dan detail
          pelaksanaannya.
        </p>
        <Link
          href={badalConsultHref}
