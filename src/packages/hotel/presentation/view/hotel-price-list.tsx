...
import { HotelNusukDisclaimerPopup } from '@/packages/hotel/presentation/view/hotel-nusuk-disclaimer-popup';
import {
  MONTH_NAMES,
  formatCompactIdr,
  formatFullIdr,
  type MonthlyPriceDetail,
} from '@/packages/hotel/domain/pricing';
import { bookingLinks } from '@/packages/hotel/domain/presentation';
...
const getPreferredBookingLink = (
  hotel: HotelWithMonthlyPrices,
): { href: string; label: string } | null => {
  return bookingLinks(hotel)[0] ?? null;
};

export const HotelPriceList = ({
  hotels,
  exchangeRate,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center text-xs text-[var(--color-text-muted)] px-1">
        ...
        {showMonthlyPrices && <span>Kurs acuan: 1 SAR = {formatFullIdr(exchangeRate)}</span>}
      </div>
                ...
                >
                  ...
                  <CardContent className="pt-0 flex-grow flex flex-col justify-between">
                    {showMonthlyPrices ? (
                          ...
                          {hotel.monthlyPrices.map((mp) => (
                            ...
                            >
                              <span className="text-[10px] text-[var(--color-text-muted)] block font-medium">
                                ...
                                {mp.isOverride && (
                                  <span className="text-[var(--color-gold)] ml-0.5 font-bold">
                                    •
                                  </span>
                                )}
                              </span>
                      ...
                      <div className="flex flex-col justify-center items-center py-6">
                        <p className="text-xs text-[var(--color-text-muted)] text-center mb-4 leading-relaxed">
                          Hubungi admin untuk mendapatkan informasi estimasi harga terbaru hotel
                          ini.
                        </p>
                        <a
                          href={(() => {
                            const adminPhone =
                              process.env.NEXT_PUBLIC_COMMUNITY_ADMIN_WHATSAPP_URL || '';
                            const cleanPhone = adminPhone.startsWith('http')
                              ? adminPhone
                              : `https://wa.me/${adminPhone}`;
                            const text = encodeURIComponent(
                              `Assalamu'alaikum Admin, saya ingin menanyakan estimasi harga terbaru untuk hotel ${hotel.label} (${hotel.city}).`,
                            );
                            return adminPhone.startsWith('http')
                              ? adminPhone
                    ...
                    <div className="mt-4 pt-3 border-t border-[var(--color-border)] flex flex-wrap justify-between items-center gap-3 text-[10px] text-[var(--color-text-muted)]">
                      {showMonthlyPrices ? (
                        <span>
                          Baseline: {hotel.sarPerNight} SAR (
                          {formatCompactIdr(hotel.sarPerNight * exchangeRate)})
                        </span>
                      ) : (
