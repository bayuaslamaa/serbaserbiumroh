...
import {
  HotelPriceList,
  type HotelWithMonthlyPrices,
} from '@/packages/hotel/presentation/view/hotel-price-list';
import { buildMonthlyPrices } from '@/packages/hotel/domain/pricing';

export const HotelNusukView = async () => {
  const [hotels, monthlyPrices, rateRows] = await Promise.all([
    db.select().from(hotelPrices).orderBy(asc(hotelPrices.city), asc(hotelPrices.label)),
