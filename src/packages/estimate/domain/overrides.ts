...
export const arePersistableEstimateTotals = (totalIdrPax: number, totalIdrGrp: number): boolean => {
...
const isCanonicalOverrideKey = (key: string): boolean => {
...
const isValidCustomRow = (v: unknown): boolean => {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const o = v as Record<string, unknown>;
  return (
...
export const validateManualOverrides = (v: unknown): v is ManualOverrides => {
...
export const normaliseAndValidateManualOverrides = (v: unknown): ManualOverrides | null => {
