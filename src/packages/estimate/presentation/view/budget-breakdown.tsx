...
import { MAX_LABEL_LEN, MAX_ROWS } from '@/packages/estimate/domain/overrides';
import {
  rp,
  rowCalc,
  exportLabel,
  ...
  travelMonthLabel,
  EXPORT_NOTES,
} from '@/packages/estimate/domain/summary';
...
const unitPrefix = (currency: string): string => {
  if (currency === 'USD') return '$';
  if (currency === 'SAR') return 'SAR';
  return 'Rp';
};

const formatUnit = (amount: number): string => {
...
const hotelFormula = (detail: HotelCostDetail): string => {
...
const buildCopyText = (display: BreakdownDisplay, pax: number, travelMonth?: number): string => {
  const lines: string[] = [
    ...
    >
      <div className="flex flex-col gap-1 min-w-0 flex-1 lg:flex-none">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {row.source !== 'overridden' && row.hotelDetail?.priceSource === 'real' && (
            <Badge>harga real</Badge>
          )}
          {row.source !== 'overridden' && row.hotelDetail?.priceSource === 'estimate' && (
            <Badge tone="muted">estimasi</Badge>
          )}
