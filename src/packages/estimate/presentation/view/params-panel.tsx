...
import type { City, EstimateParams, HotelOptionConfig, PricingConfig } from '@/shared/types';
...
import { resolveMonthlyHotelSar, sarLabel } from '@/packages/estimate/domain/hotel-pricing';
import { MONTH_LABELS } from '@/packages/estimate/domain/months';
...
export const ParamsPanel = ({ params, pricing, onChange, storySource }: ParamsPanelProps) => {
  ...
  const selectedHotelValue = (city: City): string => {
  ...
  const handleHotelChange = (city: City, hotelId: string) => {
    const patch = resolveHotelSelection(city, hotelId, pricing);
    if (patch) onChange(patch);
  };

  const hotelRadioOptions = (city: City) => {
