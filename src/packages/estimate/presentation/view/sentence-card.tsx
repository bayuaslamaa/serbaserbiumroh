...
import { availableRoomTypes, resolveRoomMultiplier } from '@/packages/estimate/domain/room-types';
...
import {
  resolveCityHotelOptions,
  resolveHotelSelection,
} from '@/packages/estimate/domain/hotel-selection';
...
const selectedHotelId = (
  city: City,
  hotelOptions: HotelOptionConfig[],
  params: EstimateParams,
): string | undefined => {
...
export const SentenceCard = ({
  ...
  const airlineLabel =
    airlineOptions.find((o) => o.value === params.airline)?.label ?? params.airline;
  ...
  const handleHotelSelect = (city: City, hotelId: string) => {
  ...
  const renderChip = (fieldKey: FieldKey, label: string, ariaLabel: string) => {
  ...
  const fieldBody = (fieldKey: FieldKey) => {
    switch (fieldKey) {
      case 'days':
        return (
          <Stepper
      ...
      case 'month':
        return (
          <MonthGrid
            ...
            onChange={(month) => onChange({ travelMonth: month })}
          />
        );
      case 'madinahNights':
        return (
          <Stepper
            value={params.nightsMadinah}
            onChange={(v) => onChange({ nightsMadinah: v })}
            min={1}
            max={30}
          />
        );
      ...
      case 'makkahNights':
        return (
          <Stepper
            value={params.nightsMakkah}
            onChange={(v) => onChange({ nightsMakkah: v })}
            min={1}
            max={30}
          />
        );
    ...
    >
      ...
      >
        ...
        {renderChip(
          'pax',
          `${params.pax} orang`,
          `Jumlah peserta: ${params.pax} orang, klik untuk ubah`,
        )}
        <span>bulan</span>
        {renderChip(
          'month',
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Semua pilihan pada kalimat ini akan direset ke default. Rincian biaya manual Anda
                tidak akan berubah.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Batal</AlertDialogCancel>
              <AlertDialogAction onClick={handleStartOverConfirm}>
                Ya, mulai ulang
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
