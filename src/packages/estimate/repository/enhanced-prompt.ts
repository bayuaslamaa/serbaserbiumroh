...
import { CARI_HOTEL_TOOL_NAME } from '@/packages/estimate/repository/tools/hotel-search';
import { HARGA_HOTEL_TOOL_NAME } from '@/packages/estimate/repository/tools/hotel-price';
...
const buildEnhancedPricingBlock = (pricing: PricingConfig): string => {
  const { rates, hotels, airlines, services } = pricing;
  const lines: string[] = [
    'Current pricing reference (hotels are NOT listed here — use your tools):',
  ];
  ...
  for (const tier of ['ECONOMY', 'STANDARD', 'PELATARAN', 'PREMIUM'] as const) {
    lines.push(
      `  ${tier}: Madinah ${hotels.MADINAH[tier].sarPerNight} SAR, Makkah ${hotels.MAKKAH[tier].sarPerNight} SAR`,
    );
  }
