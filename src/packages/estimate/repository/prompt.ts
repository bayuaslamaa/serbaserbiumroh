...
const buildDynamicPricingBlock = (pricing: PricingConfig): string => {
  ...
  if (pricing.hotelOptions) {
    ...
    for (const h of pricing.hotelOptions.MADINAH ?? []) {
      ...
      const real =
        h.realMonthlyPrices && Object.keys(h.realMonthlyPrices).length > 0 ? ', real=catalog' : '';
      lines.push(
        `  id=${h.id}, label=${h.label}, tier=${h.tier}, SAR=${h.sarPerNight}${distance}${real}, note=${h.sublabel}`,
      );
    }
