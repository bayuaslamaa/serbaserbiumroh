...
import { statusLabel } from '@/packages/community/admin-requests/domain/admin-requests-status';
...
const matchLabel = (partner: DuplicatePartner) => {
  return [
    partner.matchedByPhone ? 'nomor sama' : null,
    partner.matchedBySocial ? 'sosial sama' : null,
  ]
    .filter(Boolean)
    .join(' · ');
};

export const DuplicatePartnerPanel = ({ id, fullName, reason }: DuplicatePartnerPanelProps) => {
