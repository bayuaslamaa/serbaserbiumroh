...
export const FaqTableActions = ({ id, isPublished }: FaqTableActionsProps) => {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);

  const updatePublish = (nextPublished: boolean) => {
    setError(null);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {error && (
        <span className="basis-full text-xs" style={{ color: '#ef4444' }}>
          {error}
        </span>
      )}
    </div>
