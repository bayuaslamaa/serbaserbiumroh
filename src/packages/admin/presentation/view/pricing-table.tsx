...
import type { HotelPricingImportParseResult } from '@/packages/admin/domain/hotel-pricing-import';
import type { AirlinePricingImportParseResult } from '@/packages/admin/domain/airline-pricing-import';
...
const patch = async (category: string, body: object) => {
...
export const PricingTable = ({
  ...
  const [airlineImportPreview, setAirlineImportPreview] =
    useState<AirlinePricingImportParseResult | null>(null);
  ...
  const toggleMonthly = (hotelId: string) => {
  ...
  const toggleAirlineMonthly = (airlineId: string) => {
  ...
  const submitAddHotel = async (e: React.FormEvent) => {
    e.preventDefault();
    setAddHotelError('');
  ...
  const submitAddAirline = async (e: React.FormEvent) => {
    e.preventDefault();
    setAddAirlineError('');
    ...
    try {
      ...
      setAirlines((prev) => [
        ...prev.map((a) =>
          addAirlineForm.isDefault && a.tier === addAirlineForm.tier
            ? { ...a, isDefault: false }
            : a,
        ),
  ...
  const updateMonthlyPrice = (hotelId: string, month: number, sarPerNight: number) => {
    setHotels((prev) =>
      prev.map((h) =>
        h.id === hotelId
          ? {
              ...h,
              monthlyPrices: h.monthlyPrices.map((mp) =>
                mp.month === month ? { ...mp, sarPerNight } : mp,
              ),
            }
          : h,
      ),
  ...
  const updateAirlineMonthlyPrice = (airlineId: string, month: number, idr: number) => {
    setAirlines((prev) =>
      prev.map((a) =>
        a.id === airlineId
          ? {
              ...a,
              monthlyPrices: a.monthlyPrices.map((mp) =>
                mp.month === month ? { ...mp, idr } : mp,
              ),
            }
          : a,
      ),
  ...
  const handleAirlineImportFile = async (file: File | null) => {
  ...
  const previewAirlineImport = async () => {
    setAirlineImportError('');
    setAirlineImportPreview(null);
  ...
  const confirmAirlineImport = async () => {
    setAirlineImportError('');
    if (
  ...
  const renderHotelLinks = (hotel: HotelWithMonthly) => {
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="text-sm px-4 py-1.5 rounded border font-medium transition-colors disabled:opacity-50"
                style={{
                  borderColor: canConfirmImport ? 'var(--color-gold)' : 'var(--color-border)',
                  color: canConfirmImport ? 'var(--color-gold)' : 'var(--color-text-muted)',
                }}
              >
                      ...
                      {importPreview.rows.map((row) => (
                        ...
                        >
                          ...
                          <td className="py-1 pr-2" style={{ color: 'var(--color-text)' }}>
                            {row.data
                              ? `${row.data.city} ${row.data.tier} - ${row.data.label}${row.data.distance ? ` (${row.data.distance})` : ''}`
                              : '—'}
                          </td>
                          <td className="py-1 pr-2" style={{ color: 'var(--color-text-muted)' }}>
                            {row.errors.length > 0
                              ? row.errors.join('; ')
                              : row.status === 'update'
                                ? 'Akan memperbarui data existing'
                                : 'Siap import'}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddHotelForm((f) => ({ ...f, city: e.target.value as 'MAKKAH' | 'MADINAH' }))
                  }
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddHotelForm((f) => ({ ...f, tier: e.target.value as AddHotelForm['tier'] }))
                  }
                  className="rounded border px-2 py-1.5 text-sm bg-transparent"
                  style={{ borderColor: 'var(--color-border)', color: 'var(--color-text)' }}
                >
              ...
              <label className="flex flex-col gap-1 col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddHotelForm((f) => ({ ...f, bookingcomUrl: e.target.value }))
                  }
                    ...
                    <tr className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                      ...
                      <td className={TD}>
                        <InlineEditCell
                          ...
                          onSave={async (newVal) => {
                            ...
                            setHotels((prev) =>
                              prev.map((x) =>
                                x.id === h.id ? { ...hotel, monthlyPrices: h.monthlyPrices } : x,
                              ),
                            );
                          }}
                        />
                      ...
                      <td className={TD}>
                        <button
                          ...
                          style={{
                            borderColor: 'var(--color-border)',
                            color: isExpanded ? 'var(--color-gold)' : 'var(--color-text-muted)',
                          }}
                        >
                          ...
                          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
                            ...
                            <div>
                              ...
                              <div className="grid gap-2">
                                {(
                                  Object.entries(HOTEL_LINK_LABELS) as Array<
                                    [HotelLinkField, string]
                                  >
                                ).map(([field, label]) => (
                                  <div
                                    key={field}
                                    ...
                                    <InlineEditCell
                                      ...
                                      onSave={async (newVal) => {
                                        ...
                                        setHotels((prev) =>
                                          prev.map((x) =>
                                            x.id === h.id
                                              ? { ...hotel, monthlyPrices: h.monthlyPrices }
                                              : x,
                                          ),
                                        );
                                      }}
        ...
        {airlineImportOpen && (
          ...
          >
            ...
            <textarea
              ...
              onChange={(e) => {
                setAirlineImportCsv(e.target.value);
                setAirlineImportPreview(null);
                setAirlineImportError('');
              }}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={
                  !canConfirmAirlineImport || airlineImportConfirming || airlineImportLoading
                }
                className="text-sm px-4 py-1.5 rounded border font-medium transition-colors disabled:opacity-50"
                style={{
                  borderColor: canConfirmAirlineImport
                    ? 'var(--color-gold)'
                    : 'var(--color-border)',
                  color: canConfirmAirlineImport ? 'var(--color-gold)' : 'var(--color-text-muted)',
                }}
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddAirlineForm((f) => ({
                      ...f,
                      tier: e.target.value as AddAirlineForm['tier'],
                    }))
                  }
                  className="rounded border px-2 py-1.5 text-sm bg-transparent"
                  style={{ borderColor: 'var(--color-border)', color: 'var(--color-text)' }}
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setAddAirlineForm((f) => ({ ...f, isDefault: e.target.checked }))
                  }
                      ...
                      <td className={TD}>
                        <input
                          ...
                          onChange={async () => {
                            ...
                            const { airline } = await patch('airline', {
                              airlineId: a.id,
                              tier: a.tier,
                              isDefault: true,
                            });
                      ...
                      <td className={TD}>
                        <InlineEditCell
                          ...
                          onSave={async (newVal) => {
                            ...
                            setAirlines((prev) =>
                              prev.map((x) =>
                                x.id === a.id ? { ...airline, monthlyPrices: a.monthlyPrices } : x,
                              ),
                            );
                          }}
                        />
                ...
                <tr key={s.id} className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                  ...
                  <td className={TD}>
                    <input
                      ...
                      onChange={async (e) => {
                        try {
                          const { service } = await patch('service', {
                            key: s.key,
