...
export const AdminContentHotelsView = async () => {
  await requireAdmin();

  const hotels = await db.select().from(hotelListings).orderBy(desc(hotelListings.createdAt));
            ...
            {hotels.map((h) => (
              <tr key={h.id} className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <td className="px-4 py-3">
                  <Badge variant={h.isPublished ? 'default' : 'secondary'} className="text-xs">
                    {h.isPublished ? 'Publik' : 'Draft'}
                  </Badge>
                </td>
                <td className="px-4 py-3">
                  <HotelsTableActions id={h.id} isPublished={h.isPublished} />
                </td>
              </tr>
            ))}
          </tbody>
