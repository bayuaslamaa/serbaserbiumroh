...
import { VISITOR_BASELINE_OFFSET, formatVisitorCount } from '@/packages/stats/domain/community';
...
export const AdminVisitorStatsView = async () => {
  await requireAdmin();
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>
          Pantau jumlah kunjungan halaman publik, pengunjung unik, dan data analitik secara
          real-time.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(stats?.uniqueVisitors || 0).toLocaleString('id-ID')}
            </div>
            <p className="text-xs text-[var(--color-text-muted)]">
      ...
      <div className="grid gap-6 lg:grid-cols-5">
                ...
                <tbody className="divide-y divide-[var(--color-surface)]">
                  {pathBreakdown.length === 0 ? (
                    ...
                    pathBreakdown.map((row) => (
                      ...
                      >
                        <td
                          className="py-2 font-mono text-xs max-w-[150px] truncate"
                          title={row.path}
                        >
                          {row.path}
                        </td>
                        ...
                        <td className="py-2 text-right font-medium">
                          {row.uniques.toLocaleString('id-ID')}
                        </td>
                      </tr>
                    ))
                  )}
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <CardTitle className="text-base font-semibold flex items-center gap-2">
              <Clock className="h-4 w-4 text-[var(--color-gold)]" /> Log Kunjungan Terbaru (Maks.
              50)
            </CardTitle>
                  ...
                  {recentLogs.length === 0 ? (
                    ...
                    recentLogs.map((log) => (
                      ...
                      >
                        ...
                        <td
                          className="py-2 font-mono text-xs font-medium max-w-[120px] truncate"
                          title={log.path}
                        >
                          {log.path}
                        </td>
                        <td
                          className="py-2 font-mono text-[10px] text-[var(--color-text-muted)] max-w-[150px] truncate"
                          title={log.ipHash}
                        >
                          {log.ipHash}
                        </td>
