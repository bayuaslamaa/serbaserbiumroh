...
import type { AirlineTier } from '@/shared/types';
...
export const normalizeAirlinePricingLabel = (label: string): string => {
  return label.trim().toLowerCase().replace(/\s+/g, ' ');
};

export const normalizeAirlinePricingImportKey = (input: {
  tier: AirlineTier;
  label: string;
}): string => {
...
export const parseAirlinePricingCsv = (
  csvText: string,
  ...
  for (const airline of options.existingAirlines ?? []) {
    existingByKey.set(airline.importKey ?? normalizeAirlinePricingImportKey(airline), airline);
  }
...
const summarize = (
  rows: AirlinePricingImportRowResult[],
): Record<AirlinePricingImportStatus, number> => {
  return rows.reduce(
    (summary, row) => {
...
const emptyResult = (fileErrors: string[]): AirlinePricingImportParseResult => {
  return {
    fileErrors,
    rows: [],
