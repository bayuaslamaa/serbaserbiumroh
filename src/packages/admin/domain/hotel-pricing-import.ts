...
import type { City, HotelTier } from '@/shared/types';
...
export const normalizeHotelPricingLabel = (label: string): string => {
  return label.trim().toLowerCase().replace(/\s+/g, ' ');
};

export const normalizeHotelPricingImportKey = (input: {
  city: City;
  tier: HotelTier;
  label: string;
}): string => {
...
export const parseHotelPricingCsv = (
  csvText: string,
...
const parseRecord = (
  ...
  const agodaUrl = normalizeOptionalUrl(record.agoda_url);
  const bookingcomUrl = normalizeOptionalUrl(record.bookingcom_url);
  const tripcomUrl = normalizeOptionalUrl(record.tripcom_url);
  const bookingUrl = normalizeOptionalUrl(record.booking_url);
  ...
  if (agodaUrl === false) errors.push('agoda_url must be a valid http/https URL when provided');
  if (bookingcomUrl === false)
    errors.push('bookingcom_url must be a valid http/https URL when provided');
  if (tripcomUrl === false) errors.push('tripcom_url must be a valid http/https URL when provided');
  if (bookingUrl === false) errors.push('booking_url must be a valid http/https URL when provided');
...
const normalizeEnumValue = (value: string | undefined): string => {
...
const normalizeOptionalUrl = (value: string | undefined): string | null | false => {
  const normalized = (value ?? '').trim();
  if (!normalized) return null;

  try {
    const parsed = new URL(normalized);
    return parsed.protocol === 'http:' || parsed.protocol === 'https:' ? normalized : false;
  } catch {
    return false;
  }
};
...
const summarize = (
  rows: HotelPricingImportRowResult[],
): Record<HotelPricingImportStatus, number> => {
  return rows.reduce(
    (summary, row) => {
...
const emptyResult = (fileErrors: string[]): HotelPricingImportParseResult => {
  return {
    fileErrors,
    rows: [],
