...
import type { AirlineTier, HotelTier } from '@/shared/types';
...
export const normalizePilgrimStorySlug = (slug: string): string => {
  return slug.trim().toLowerCase();
};

export const parsePilgrimStoryCsv = (
  csvText: string,
  ...
  const rows = parsedRows.map((row): PilgrimStoryImportRowResult => {
    ...
    if (errors.length > 0) {
      return {
        ...row,
        status: errors.some((error) => error.startsWith('duplicate')) ? 'conflict' : 'invalid',
        errors,
      };
    }
...
const parseRecord = (
  ...
  const totalBudgetIdr = requiredInteger(record.total_budget_idr, 'total_budget_idr', errors, {
    min: 0,
  });
  const travelMonth = optionalInteger(record.travel_month, 'travel_month', errors, {
    min: 1,
    max: 12,
  });
  const travelYear = optionalInteger(record.travel_year, 'travel_year', errors, { min: 1 });
  const makkahNights =
    optionalInteger(record.makkah_nights, 'makkah_nights', errors, { min: 0 }) ?? 0;
  const madinahNights =
    optionalInteger(record.madinah_nights, 'madinah_nights', errors, { min: 0 }) ?? 0;
...
const parseHotelTier = (value: string | undefined, errors: string[]): HotelTier | null => {
...
const parseOptionalAirlineTier = (
  value: string | undefined,
  errors: string[],
): AirlineTier | null => {
  const tier = value?.trim().toUpperCase() ?? '';
...
const summarize = (
  rows: PilgrimStoryImportRowResult[],
): Record<PilgrimStoryImportStatus, number> => {
  return rows.reduce(
    (summary, row) => {
...
const emptyResult = (fileErrors: string[]): PilgrimStoryImportParseResult => {
  return {
    fileErrors,
    rows: [],
