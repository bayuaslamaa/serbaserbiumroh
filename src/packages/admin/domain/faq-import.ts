...
export const normalizeFaqImportText = (value: string): string => {
  return value.trim().toLowerCase().replace(/\s+/g, ' ');
};

export const parseFaqCsv = (
  csvText: string,
  ...
  const parsedRows = records.map((record, index) =>
    parseRecord(record, index + 2, existingGroupsByKey),
  );
...
const summarize = (rows: FaqImportRowResult[]): Record<FaqImportStatus, number> => {
  return rows.reduce(
    (summary, row) => {
...
const emptyResult = (fileErrors: string[]): FaqImportParseResult => {
  return {
    fileErrors,
    rows: [],
