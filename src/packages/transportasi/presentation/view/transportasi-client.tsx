...
const TransportasiClient = () => {
  ...
  const filteredRoutes = useMemo(() => {
    ...
    return activeVehicle.routes.filter(
      (route) => route.from.toLowerCase().includes(query) || route.to.toLowerCase().includes(query),
    );
  ...
  return (
    <div className="mx-auto max-w-6xl py-8 md:py-12 px-2">
      <section className="text-center mb-12 max-w-3xl mx-auto">
        ...
        >
          Kami menyediakan armada ber-AC terbaik dengan driver profesional & berpengalaman di Arab
          Saudi. Melayani penjemputan bandara, antar jemput hotel, ziarah Makkah-Madinah, hingga
          ziarah kota Taif.
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 mt-8">
          <div className="flex flex-col items-center p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            <span className="text-[10px] text-[var(--color-text-muted)] text-center hidden sm:inline">
              Armada bersih & AC dingin
            </span>
          </div>
          <div className="flex flex-col items-center p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            <span className="text-[10px] text-[var(--color-text-muted)] text-center hidden sm:inline">
              Driver standby sebelum kedatangan
            </span>
          </div>
          <div className="flex flex-col items-center p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          <div className="md:col-span-7 space-y-3">
            ...
            <ul className="text-xs md:text-sm space-y-2 text-[var(--color-text-muted)] pl-5 list-disc">
              <li>
                Semua tarif dasar pada daftar di bawah merupakan **harga final (nett)** untuk satu
                armada kendaraan.
              </li>
              <li>
                Untuk rute{' '}
                <strong className="text-[var(--color-gold)]">Penjemputan di Bandara</strong> (Jeddah
                Airport & Madinah Airport), tarif sudah otomatis termasuk **Airport Pickup Fee / Tip
                (+50 SAR)**.
              </li>
              <li>
                Estimasi Rupiah (IDR) disesuaikan secara real-time berdasarkan nilai kurs yang Anda
                masukkan di kalkulator.
              </li>
            </ul>
          </div>
      ...
      <section className="mb-6 space-y-4">
            ...
            {vehicles.map((v) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  {v.icon}
                  <span className="font-bold text-sm md:text-base text-[var(--color-text)]">
                    {v.name}
                  </span>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-[var(--color-text-muted)] block flex items-center gap-1">
                    <Users className="w-3 h-3 shrink-0 text-[var(--color-gold)]" /> {v.passengers}{' '}
                    Penumpang
                  </span>
        ...
        <div className="p-4 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            ...
            <p className="text-xs text-[var(--color-text-muted)]">{activeVehicle.details}</p>
          </div>
        ...
        {filteredRoutes.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-[var(--color-border)] rounded-xl">
            ...
            <p className="text-xs text-[var(--color-text-muted)]">
              Tidak ada rute yang cocok dengan kata pencarian &quot;{searchQuery}&quot; pada armada
              ini.
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-end bg-black/30 p-3 rounded-lg border border-[var(--color-border)]">
                      <div>
                        <span className="text-[10px] text-[var(--color-text-muted)] block uppercase tracking-wider font-semibold">
                          Total Tarif SAR:
                        </span>
                        <span className="text-2xl font-black text-[var(--color-gold)] font-mono">
                          {finalPriceSAR} SAR
                        </span>
                      </div>
      ...
      <section className="mb-12 p-6 rounded-lg border border-[var(--color-border)] bg-black/25">
        <h3
          className="text-lg font-bold mb-4 flex items-center gap-2 text-[var(--color-gold)]"
          ...
          <div className="space-y-3">
            <div>
              <h5 className="font-bold text-[var(--color-text)] mb-1">
                Penjemputan Bandara (Airport Pickup)
              </h5>
              <p style={{ color: 'var(--color-text-muted)' }}>
                Driver kami akan memantau status penerbangan Anda secara berkala. Driver akan
                menunggu Anda di dekat area pintu keluar kedatangan dengan membawa papan
                nama/identitas yang disepakati untuk kemudahan pertemuan.
              </p>
            </div>
            <div>
              <h5 className="font-bold text-[var(--color-text)] mb-1">
                Kapasitas Bagasi & Penumpang
              </h5>
              <p style={{ color: 'var(--color-text-muted)' }}>
                Demi keselamatan dan kenyamanan berkendara, mohon pastikan barang bawaan (koper) dan
                jumlah personel Anda tidak melebihi kapasitas maksimal yang tertera pada
                masing-masing tipe kendaraan.
              </p>
            </div>
          </div>
          <div className="space-y-3">
            <div>
              <h5 className="font-bold text-[var(--color-text)] mb-1">
                Rute Khusus & Ziarah Tambahan
              </h5>
              <p style={{ color: 'var(--color-text-muted)' }}>
                Apabila Anda membutuhkan rute kustom atau ziarah tambahan yang belum tercantum di
                brosur tarif, silakan konsultasikan secara langsung dengan admin untuk mendapatkan
            ...
            <div>
              ...
              <p style={{ color: 'var(--color-text-muted)' }}>
                Semua booking yang dilakukan melalui tombol WhatsApp akan diproses secara manual
                oleh admin SSU Tour. Anda akan menerima bukti pemesanan formal setelah detail rute
                dan jam penjemputan terkonfirmasi.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="text-center py-8 px-4 rounded-xl border border-[var(--color-gold-muted)] bg-gradient-to-b from-[rgba(201,168,76,0.05)] to-transparent max-w-3xl mx-auto">
        <h3
          className="text-2xl font-bold mb-3 text-[var(--color-gold)]"
          style={{ fontFamily: 'var(--font-heading)' }}
        >
        ...
        <p
          ...
          style={{ color: 'var(--color-text-muted)' }}
        >
          Hubungi admin kami di WhatsApp untuk memesan rute kustom, grup besar (bus), atau
          koordinasi kedatangan rombongan umroh mandiri Anda.
        </p>
...
export default TransportasiClient;
