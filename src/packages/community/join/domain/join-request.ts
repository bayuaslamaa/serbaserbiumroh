...
export type CommunityJoinValidationResult =
  | { success: true; data: ParsedCommunityJoinRequest }
...
export function parseCommunityJoinInput(input: CommunityJoinInput): CommunityJoinValidationResult {
  ...
  if (socialUsername.length > MAX_SOCIAL_LENGTH) {
    return { success: false, error: `Username sosial media maksimal ${MAX_SOCIAL_LENGTH} karakter` }
  }
