...
const PANDUAN_DIR = path.join(process.cwd(), 'src', 'shared', 'content', 'panduan');
...
export const getAllGuides = (): GuideMetadata[] => {
  ...
  const groups = fs
    .readdirSync(PANDUAN_DIR, { withFileTypes: true })
...
export const getGuideBySlug = (slug: string): GuideMetadata | null => {
