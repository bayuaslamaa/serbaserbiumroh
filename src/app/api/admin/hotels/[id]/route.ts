...
export const GET = async (_req: NextRequest, ctx: RouteCtx) => {
  ...
  const [hotel] = await db.select().from(hotelListings).where(eq(hotelListings.id, id)).limit(1);
...
export const PUT = async (req: NextRequest, ctx: RouteCtx) => {
  ...
  if (typeof tier === 'string')
    hotelData.tier = tier as 'ECONOMY' | 'STANDARD' | 'PELATARAN' | 'PREMIUM';
...
export const DELETE = async (_req: NextRequest, ctx: RouteCtx) => {
  ...
  const [deleted] = await db.delete(hotelListings).where(eq(hotelListings.id, id)).returning();
