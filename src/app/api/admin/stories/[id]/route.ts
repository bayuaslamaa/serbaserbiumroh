...
export async function PUT(req: NextRequest, ctx: RouteCtx) {
  ...
  if (typeof hotelTier === "string") storyData.hotelTier = hotelTier as "ECONOMY" | "STANDARD" | "PELATARAN" | "PREMIUM"
  if (airlineTier !== undefined) storyData.airlineTier = airlineTier as "BUDGET" | "STANDARD" | "GARUDA" | "BUSINESS" | null
  ...
  const days = Array.isArray(itineraryDays) ? itineraryDays as Array<Record<string, unknown>> : null
  const items = Array.isArray(packingItems) ? packingItems as Array<Record<string, unknown>> : null
