...
import { applyOverrides, isEmptyOverrides } from '@/packages/estimate/domain/budget/overrides';
import {
  normaliseAndValidateEstimateParams,
  validateEstimateHotelIds,
} from '@/packages/estimate/domain/params';
import {
  arePersistableEstimateTotals,
  normaliseAndValidateManualOverrides,
...
export const PATCH = async (req: NextRequest, ctx: RouteContext) => {
  const session = await auth();
  if (!session?.user) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }
  ...
  if (body.manualOverrides !== undefined) {
    ...
    if (body.manualOverrides !== null)
      normalisedBodyOverrides = normaliseAndValidateManualOverrides(body.manualOverrides);
  ...
  const mutatesEstimate =
    body.params !== undefined || body.title !== undefined || body.manualOverrides !== undefined;
  if (!mutatesEstimate) {
  ...
  if (body.expectedUpdatedAt !== undefined) {
    ...
    if (estimate.updatedAt.getTime() !== expectedUpdatedAt.getTime()) {
      return NextResponse.json(
        { error: 'Estimate was modified by another session' },
        { status: 409 },
      );
    }
  }
  ...
  if (body.params !== undefined || body.manualOverrides !== undefined) {
    ...
    const display = applyOverrides(
      calculateBudget(effectiveParams, pricing),
      effectiveOverrides,
      effectiveParams.pax,
    );
    if (!arePersistableEstimateTotals(display.totalIdrPax, display.totalIdrGrp)) {
      return NextResponse.json(
        { error: 'estimate total exceeds supported range' },
        { status: 400 },
      );
    }
