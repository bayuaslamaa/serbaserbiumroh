...
export function FaqGroupForm({ groups }: FaqGroupFormProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [newName, setNewName] = useState("")
  const [newSortOrder, setNewSortOrder] = useState("0")
  const [drafts, setDrafts] = useState(() =>
    Object.fromEntries(groups.map((group) => [group.id, { name: group.name, sortOrder: String(group.sortOrder) }]))
  )
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {groups.map((group) => {
          const draft = drafts[group.id] ?? { name: group.name, sortOrder: String(group.sortOrder) }
          return (
            <div key={group.id} className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_120px_auto_auto]">
              <input
                className={inputClass}
                style={inputStyle}
                value={draft.name}
