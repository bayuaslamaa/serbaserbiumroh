...
export function StoryForm({ initialData }: StoryFormProps) {
  ...
  const [totalBudgetIdr, setTotalBudgetIdr] = useState(initialData?.totalBudgetIdr?.toString() ?? "0")
  ...
  function addItem() {
    setPackingItems((prev) => [
      ...prev,
      { category: "", itemName: "", sortOrder: prev.length },
    ])
  }
  ...
  function updateItem(index: number, field: keyof PackingItem, value: string | number) {
    setPackingItems((prev) =>
