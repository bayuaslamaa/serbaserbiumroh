...
import { MegaPanel } from '@/components/molecules/nav-dropdown';
import { isExternalHref, serviceCardTreatment, services } from '@/packages/layanan/domain/catalog';

export const LayananTrigger = ({ isOpen, onToggle }: { isOpen: boolean; onToggle: () => void }) => {
  return (
    <button
      type="button"
      onClick={onToggle}
      ...
      style={{
        ...
        borderColor: isOpen ? 'var(--color-gold-muted)' : 'rgba(201,168,76,0.25)',
      }}
    >
      Layanan
      <ChevronDown
...
export const LayananPanel = ({ onNavigate }: { onNavigate: () => void }) => {
  return (
    <MegaPanel>
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-1.5 block text-xs font-bold text-gold">{service.price}</span>
                </span>
              </Link>
            );
          })}
