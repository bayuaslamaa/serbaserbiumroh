...
// Eleven checkboxes in one flat column hides the ziyarah and muthowif rows among five near-identical
// route names. The legs are fenced off under their own heading instead — the same section + small
// caps heading ParamsPanel already uses for its field groups, one level down since that heading (or
// the field tray's) sits above this grid.
//
// Order and membership both come from SERVICE_KEYS: this component keeps no list of its own, so a
// key added to the catalogue is a key the operator can tick. The legs are contiguous there, so
// grouping consecutive runs preserves the catalogue order exactly.
interface ServiceGroup {
  label?: string
  keys: ServiceKey[]
}

function groupServices(keys: ServiceKey[]): ServiceGroup[] {
...
export function ServiceCheckboxGrid({ pricing, value, onChange }: ServiceCheckboxGridProps) {
  ...
  function renderCheckbox(key: ServiceKey) {
        ...
        group.label ? (
          <section key={group.label} className="flex flex-col gap-2 pt-1">
            ...
            <p className="text-[11px]" style={{ color: "var(--color-text-muted)" }}>
              Satu perjalanan mendarat di Jeddah sekali dan pulang sekali — pilih satu penjemputan dan
              satu pengantaran.
            </p>
